    ...
    const events = [
      ...
      return (
        <div className="container">
          <div className="lakers-wrapper">
            <h1 className='lakers-h1'>LA Lakers Schedule</h1>
            <ul className="sessions">
              {events.map((event, index) => (
                <li key={index} className="lakers-times" onClick={() => onEventClick(event)}>
                  <div className="lakers-time">{event.date}</div>
                  <p className='lakers-p'>{event.description}</p>
                </li>
              ))}
            </ul>
          </div>
