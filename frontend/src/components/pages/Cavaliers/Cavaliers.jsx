    ...
    const events = [
      ...
      return (
        <div className="container">
          <div className="cavaliers-wrapper">
            <h1 className='cavaliers-h1'>Cleveland Cavaliers Schedule</h1>
            <ul className="sessions">
              {events.map((event, index) => (
                <li key={index} className="cavaliers-times" onClick={() => onEventClick(event)}>
                  <div className="cavaliers-time">{event.date}</div>
                  <p className='cavaliers-p'>{event.description}</p>
                </li>
              ))}
            </ul>
          </div>
