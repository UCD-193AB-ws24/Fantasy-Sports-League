    ...
    const events = [
      ...
      return (
        <div className="container">
          <div className="knicks-wrapper">
            <h1 className='knicks-h1'>New York Knicks Schedule</h1>
            <ul className="sessions">
              {events.map((event, index) => (
                <li key={index} className="knicks-times" onClick={() => onEventClick(event)}>
                  <div className="knicks-time">{event.date}</div>
                  <p className='knicks-p'>{event.description}</p>
                </li>
              ))}
            </ul>
          </div>
