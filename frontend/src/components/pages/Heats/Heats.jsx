    ...
    const events = [
      ...
      return (
        <div className="container">
          <div className="heats-wrapper">
            <h1 className='heats-h1'>Miami Heats Schedule</h1>
            <ul className="sessions">
              {events.map((event, index) => (
                <li key={index} className="heats-times" onClick={() => onEventClick(event)}>
                  <div className="heats-time">{event.date}</div>
                  <p className='heats-p'>{event.description}</p>
                </li>
              ))}
            </ul>
          </div>
