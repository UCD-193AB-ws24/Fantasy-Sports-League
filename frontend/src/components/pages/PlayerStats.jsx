...
function PlayerStatsModal({ player, onClose, isRoster = false, onPlayerDropped = () => {}, leagueId = null }) {
  ...
  useEffect(() => {
    if (!player || !player.id) {
      console.warn("Invalid player passed to PlayerStats:", player);
      return;
    }
  
    fetch(`http://localhost:5001/api/players/${player.id}/gamelogs`, {
      credentials: 'include'
    })
  ...
  const handleDropPlayer = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5001/api/roster/removePlayer', {
        method: 'DELETE',
      ...
      alert(data.message);
      onPlayerDropped(player.id);  // 🔁 Immediately notify parent component
      onClose();
    } catch (error) {
      console.error("Error dropping player:", error);
      alert(error.message || "Error dropping player");
    } finally {
      setDropping(false);
  ...
  // Team color configuration (keeping your existing team colors)
  ...
  return (
    <>
      ...
      {showDropConfirm && (
        <div className="PS_modal-overlay" style={{ zIndex: 11000 }}>
          <div className="PS_drop-confirm-dialog">
            <h3>Drop Player Confirmation</h3>
            <p>Are you sure you want to drop <strong>{player.name}</strong> from your roster?</p>
            <p className="PS_warning-text">This action cannot be undone. The player will become available for other teams to add.</p>
            <div className="PS_confirm-buttons">
              <button 
                className="PS_cancel-drop-btn" 
