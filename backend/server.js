...
// Then initialize Socket.IO directly on the HTTP server
...
// Track active users with Socket.IO instead
...
app.get('/api/roster/:userId/:leagueId', async (req, res) => {
  try {
    // Ensure userId exists and is converted to number
    ...
    // Find roster with proper where clause
    let roster = await prisma.roster.findFirst({
      where: { 
        userId: userId,
        leagueId: leagueId
    ...
    // If no roster found with specific leagueId, try without league constraint
    if (!roster) {
      console.log(`No roster found for user ${userId} in league ${leagueId}, trying without league ID`);
      roster = await prisma.roster.findFirst({
        where: { userId: userId },
        include: { 
          players: {
            include: { 
    ...
    // Return what was found, or empty data
    res.json(roster || { userId, players: [] });
  } catch (error) {
    console.error("Error fetching roster:", error);
    res.status(500).json({ error: "Failed to get roster: " + error.message });
  }
});
...
app.post('/api/roster/add', authenticate, async (req, res) => {
  try {
    ...
    const { playerId, position, isBench, leagueId } = req.body;
    ...
    if (leagueId) {
      // If leagueId is provided, find or create the league-specific roster
      roster = await prisma.roster.findFirst({
        where: { 
      ...
      if (!roster) {
        // Create a new roster for this league
        roster = await prisma.roster.create({
          data: {
            userId: parseInt(userId),
            leagueId: parseInt(leagueId),
      ...
      // Fallback to finding any roster for the user (backward compatibility)
      roster = await prisma.roster.findFirst({
    ...
    // Count starters and bench players for THIS specific roster
  ...
  app.delete('/api/roster/removePlayer', authenticate, async (req, res) => {
    try {
      ...
      const leagueId = req.body.leagueId ? parseInt(req.body.leagueId) : null;
  
      console.log("Drop Request Received:", { userId, playerId, leagueId });
      ...
      let roster;
      if (leagueId) {
        // If leagueId is provided, find the specific league roster
        roster = await prisma.roster.findFirst({
          where: { 
            userId: userId,
            leagueId: leagueId
        ...
        // Fallback to any roster (for backward compatibility)
        roster = await prisma.roster.findFirst({
          where: { userId: userId },
          include: {
            players: true
      ...
      // Find the player on this specific roster
      const playerOnRoster = roster.players.find(p => p.playerId === playerId);
  
      if (!playerOnRoster) {
        return res.status(400).json({ error: "Player is not on your roster" });
      }
  
      // Delete the player from the roster
      await prisma.rosterPlayer.delete({
        where: { id: playerOnRoster.id }
      });
  
      console.log(`Successfully dropped player ${playerId} from roster ${roster.id}`);
      ...
      res.status(500).json({ error: "Failed to drop player: " + error.message });
    }
  });
...
app.get('/api/roster/forBots/:userId/playerNames', async (req, res) => {
  try {
    // Extract userId as string from params
    ...
    // Validate userId
    if (isNaN(userId)) {
      console.error(`Invalid userId parameter: ${userIdStr}`);
      return res.status(400).json({ error: "Invalid user ID format" });
    }
    
    console.log(`Fetching roster player names for user ${userId}`);
    
    // Find the roster with proper where clause
    const roster = await prisma.roster.findFirst({
      where: { userId: userId },
      include: {
        players: {
    ...
    if (!roster) {
      console.log(`No roster found for user ${userId}`);
      return res.json({ playerNames: [] });
    }
    
    // Extract player names and relevant data from roster
    ...
    console.log(`Found ${playerNames.length} players for user ${userId}`);
    res.json({ playerNames });
  } catch (error) {
    console.error(`Error fetching roster player names: ${error.message}`);
    res.status(500).json({ 
...
app.post('/api/leagues/:leagueId/draft/pick', authenticate, async (req, res) => {
  try {
    ...
    // Emit a "pick-in-progress" event to show the user is making a selection
...
// Reset draft (for commissioner only)
app.post('/api/leagues/:leagueId/draft/reset', authenticate, async (req, res) => {
  try {
    const leagueId = parseInt(req.params.leagueId);
    const userId = req.user.id;
    ...
    if (league.commissionerId !== userId.toString()) {
      return res.status(403).json({ error: "Only commissioner can reset draft" });
    }
    ...
    res.json({ success: true, message: "Draft has been reset successfully" });
  } catch (error) {
...
app.get('/api/leagues/:leagueId/teams', authenticate, async (req, res) => {
  try {
    ...
    // Build team roster data from actual rosters
    const teams = [];
    
    for (const user of league.users) {
      // Get the actual roster for this user in this league
      const roster = await prisma.roster.findFirst({
        where: { 
          userId: user.id,
          leagueId: leagueId
      ...
      // Map roster players to the format expected by frontend
      const players = roster ? roster.players.map(rp => ({
        id: rp.player.id,
        name: rp.player.name,
        team: rp.player.team,
...
// Store messages by league ID
const leagueMessages = {};


// Socket.IO event handlers for chat and draft
io.on("connection", (socket) => {
console.log("New Socket.IO connection:", socket.id);

// Handle joining a league chat room
socket.on('join-chat', ({ leagueId }) => {
  console.log(`User joined league chat: ${leagueId}`);
  socket.join(`chat-${leagueId}`);
  
  // Send chat history for this league
  if (leagueMessages[leagueId]) {
    socket.emit('chat-history', leagueMessages[leagueId]);
  }
});

// Handle new messages
socket.on('send-message', (messageData) => {
  const { leagueId } = messageData;
  
  console.log(`New message in league ${leagueId}:`, messageData.text);
  
  // Initialize messages array for this league if it doesn't exist
  if (!leagueMessages[leagueId]) {
    leagueMessages[leagueId] = [];
  }
  
  // Add message to history (limit to last 100 messages)
  leagueMessages[leagueId].push(messageData);
  if (leagueMessages[leagueId].length > 100) {
    leagueMessages[leagueId].shift();
  }
  
  // Broadcast the message to all clients in the league chat room
  io.to(`chat-${leagueId}`).emit('message', messageData);
});

// Join draft room handler
socket.on('join-draft', ({ leagueId }) => {
  console.log(`User joined draft room: ${leagueId}`);
  socket.join(`draft-${leagueId}`);
});

// Leave draft room handler
socket.on('leave-draft', ({ leagueId }) => {
  console.log(`User left draft room: ${leagueId}`);
...
async function handleAutoPick(leagueId, draftId) {
  try {
    ...
    // First emit a "pick-in-progress" event to show the bot is making a selection
    io.to(`draft-${leagueId}`).emit('pick-in-progress', {
    ...
    // Small delay to show the "picking" state
...
server.listen(5002, () => {
  console.log(`Socket.IO server is running on http://localhost:5002`);
});
