...
def download_player_headshots():
    """Download headshots for all NBA players."""
    import os
    import requests
    
    # Create headshots directory if it doesn't exist
    if not os.path.exists('public/headshots'):
        os.makedirs('public/headshots')
    
    # Create a default image for players without headshots
    if not os.path.exists('public/headshots/default.jpg'):
        default_url = "https://cdn.nba.com/headshots/nba/latest/260x190/fallback.png"
        response = requests.get(default_url)
        with open('public/headshots/default.jpg', 'wb') as f:
            f.write(response.content)
    
    # Read player names from file
    with open('parsed_nba_players.txt', 'r') as f:
        player_names = f.read().strip().split('\n')
    
    print(f"Starting download of {len(player_names)} player headshots...")
    
    # Loop through each player
    count = 0
    for player_name in player_names:
        safe_name = player_name.replace(' ', '_')
        file_path = f'public/headshots/{safe_name}_headshot.jpg'
        ...
        if player_id:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"No headshot found for {player_name}")
            except Exception as e:
                print(f"Error downloading headshot for {player_name}: {e}")
        else:
            print(f"Could not find player ID for {player_name}")
    
    print(f"Downloaded {count} player headshots")

def main():
    """Process live stats request"""
    if len(sys.argv) < 3:
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print(json.dumps({"error": "Invalid arguments. Expected 'command [args]'"}))
        sys.exit(1)
        
    command = sys.argv[1].strip().lower()
    
    if command == "download_headshots":
        download_player_headshots()
    elif command == "common_info" and len(sys.argv) >= 3:
        player_name = " ".join(sys.argv[2:]).strip()
        ...
        print(json.dumps({"error": f"Invalid command '{command}'. Valid commands are: 'download_headshots', 'common_info', 'live_stats'"}))
        sys.exit(1)