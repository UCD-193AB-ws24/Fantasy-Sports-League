...
async function updatePlayerData(leaguePlayer) {
  try {
    ...
    for (const gameStats of gameLogs) {
      ...
      // Calculate fantasy points as a float, then round to 2 decimals.
      const rawFantasyPoints =
        (gameStats.points || 0) +
      ...
      // Round to 2 decimals
      const fantasyPoints = parseFloat(rawFantasyPoints.toFixed(2));
      ...
      const newData = {
        ...
        ft: gameStats.ft || "0/0",
        // Store the minutes exactly as provided (string), default to "0" if missing
        minutes: gameStats.minutes ?? "0",
        fantasyPoints: fantasyPoints,
      };
      //console.log(`New data for ${player.name} on ${gameStats.date}:`, newData);
